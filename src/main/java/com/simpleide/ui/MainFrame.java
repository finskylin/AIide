...
public class MainFrame extends JFrame {
            ...
            if (selectedText != null && !selectedText.trim().isEmpty()) {
                try {
                    ...
                    floatingButton.setBounds(
                        layeredPane.getWidth() - buttonWidth - margin,  // 右边距离
                        margin,  // 上边距离
                        buttonWidth,
                        buttonHeight
                    );
    ...
    private void createNewDirectory() {
        ...
        if (dirName != null && !dirName.trim().isEmpty()) {
            String targetDir = currentFile != null ? 
                currentFile.getParent() : projectRoot;
    ...
    private void addFileContent(StringBuilder content, String filePath) {
        try {
            String fileContent = new String(Files.readAllBytes(
                Paths.get(projectRoot, filePath)));
    ...
    private void initShortcuts() {
        ...
        inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_S, InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK), "saveAs");
        ...
        inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_F, InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK), "format");
    ...
    private void initEditorPopupMenu() {
        ...
        addMenuItem(popupMenu, "另存为", KeyEvent.VK_S, InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK, e -> saveFileAs());
        popupMenu.addSeparator();
        ...
        // 代码操作
        addMenuItem(popupMenu, "格式化", KeyEvent.VK_F, InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK, e -> formatCurrentFile());
    ...
    // 添加代码修改解析和应用方法
    private void parseAndApplyCodeModifications(String response, List<String> filePaths) {
        // 首先提取所有文件路径
        ...
        if (message.getRole() == ChatMessage.Role.AI) {
            ...
            for (int i = 1; i < blocks.length; i += 2) {
                if (i < blocks.length) {
                    ...
                    // 从文件路径映射中获取完整路径
                    String fileName = getFileNameFromPath(header.contains(":") ? header.split(":", 2)[1].trim() : header);
    ...
    private void showProjectSettings() {
        ...
        okButton.addActionListener(e -> {
            // 保存设置
            saveProjectSettings(
                sourceVersion.getSelectedItem().toString(),
                targetVersion.getSelectedItem().toString(),
                encoding.getSelectedItem().toString(),
                indentType.getSelectedItem().toString(),
    ...
    private void saveProjectSettings(String sourceVersion, String targetVersion, 
                                   String encoding, String indentType, int indentSize) {
        try {
            ...
            try (FileOutputStream out = new FileOutputStream(
                new File(settingsFile, "project.properties"))) {
            ...
            for (Component comp : components) {
                if (comp instanceof JScrollPane) {
                    ...
                    if (button.isVisible()) {
                        ...
                        int margin = 10;
                        button.setBounds(
                            parent.getWidth() - buttonWidth - margin,
                            margin,
                            buttonWidth,
                            buttonHeight
                        );
